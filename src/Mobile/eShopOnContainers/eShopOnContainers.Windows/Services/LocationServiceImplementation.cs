...
using bliztafree.Core.Services.Location;
using bliztafree.Windows.Services;
using System;
...
namespace bliztafree.Windows.Services
{
    public class LocationServiceImplementation : ILocationServiceImplementation
    {
        ...
        {
            var timeoutMilliseconds = timeout.HasValue ? (int)timeout.Value.TotalMilliseconds : bliztafree.Windows.Helpers.Timeout.Infinite;
            if (timeoutMilliseconds < 0 && timeoutMilliseconds != bliztafree.Windows.Helpers.Timeout.Infinite)
            ...
            var timer = new bliztafree.Windows.Helpers.Timeout(timeoutMilliseconds, pos.Cancel);
