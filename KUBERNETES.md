...
>`deployments.yaml`
>
>The first container runs the `bliztafree/basket.api` image:
>```yaml
>spec:
>  containers:
>  - name: basket
...
### Services
Services expose Pods to external networks. For example, the `basket` Service exposes Pods with labels `app=bliztafree` and `component=basket` to the cluster at large:
>`services.yaml`
>```yaml
>kind: Service
>metadata:
